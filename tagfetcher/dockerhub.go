...
func FetchTags(imageName string) ([]ImageTag, error) {
	url := fmt.Sprintf("https://registry.hub.docker.com/v2/namespaces/library/repositories/%s/tags?page_size=30", imageName)
	...
	for _, result := range tagsResp.Results {
		tags = append(tags, ImageTag{
			Name:          result.Name,
			TagLastPushed: result.TagLastPushed,
